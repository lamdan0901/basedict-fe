...
import { cn } from "@/lib";
import { MeaningInDetail } from "@/modules/home/TranslationSection/JPMeaningSection/MeaningInDetail";
import { MeaningInPreview } from "@/modules/home/TranslationSection/JPMeaningSection/MeaningInPreview";
...
export const JPMeaningSection = forwardRef((_, ref) => {
  const [searchParam] = useQueryParam("searchVietnamese", "");
  const searchText = useVnToJpTransStore((state) => state.searchText);
  ...
  const translateJPToVN = useCallback(async () => {
    const data = await translateJPToVNTrigger();
    ...
    <>
      ...
      <Card className={cn("w-full rounded-2xl min-h-[328px] h-fit relative ")}>
        <CardContent className="!p-4 space-y-1">
          ...
          ) : rawTranslatedLexemes.length > 0 || lexemeSearch ? (
            <>
              <MeaningInPreview
              ...
              >
                <CircleChevronDown
                  className={cn("size-5 mr-2", showMeaning ? "rotate-180" : "")}
                />
